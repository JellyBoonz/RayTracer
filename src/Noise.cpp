...
// Permutation table
const int Noise::permutation[] = {151, 160, 137, 91, 90, 15,
                                  ...
                                  78, 66, 215, 61, 156, 180, 151, 160, 137, 91, 90, 15};

std::vector<int> Noise::p;

void Noise::initializePermutation()
{
    if (p.empty())
    {
        p.resize(512);
...
double Noise::perlinNoise(const Tuple &point)
{
...
std::vector<double> Noise::generateNoiseRange(const Tuple &point, int count)
{
    std::vector<double> noiseValues;
    for (int i = 0; i < count; ++i)
    {
        Tuple offsetPoint = Tuple::point(point.x + i, point.y + i, point.z + i);
        noiseValues.push_back(perlinNoise(offsetPoint));
    }
    return noiseValues;
}